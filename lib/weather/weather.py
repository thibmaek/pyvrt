...
def __get_zone_from_list(items_list, zone):
    try:
        res = filter(
            lambda item: item['location'].lower() == zone.lower(),
            items_list
        )
        return res
...
def current(region='BE', zone=None):
    if region is 'BE':
        res = vrt_api_request(
            ...
            params={
              'accept': 'application/vnd.weather.vrt.be.observations_1.0+json'
            }
        )
...
def forecast(region='BE', zone=None):
    if region is 'BE':
        res = vrt_api_request(
            ...
            params={
              'accept': 'application/vnd.weather.vrt.be.forecasts_1.0+json'
            }
        )
